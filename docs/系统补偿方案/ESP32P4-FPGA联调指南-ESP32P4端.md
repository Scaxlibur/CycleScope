...
- [CSLP UDP 通信协议 v0.1](../协议与接口/CSLP-UDP-通信协议-v0.1.md)
- [CSLP G题采样与处理 Profile v0.1](../协议与接口/CSLP-G题采样与处理-Profile-v0.1.md)
- [AD9226 通道 A 与 Zynq-7010 接线定义](../协议与接口/AD9226通道A与Zynq-7010接线定义.md)
...
- 正式 P4 `.3:50001` 与 FPGA `.2:50000` 已完成真实 `HELLO → CONFIG_SET → ENABLE_PUSH → measurement → UI LIVE` 链路、连续 600 帧回归及 M12 的 60 个工程观测点；适用范围和原始根见 [测试与证据索引](../测试与证据索引.md)。
- P4 接收、重组、三缓冲、ANSI FFT、测量、时域/频域投影、LVGL bridge、生命周期回滚和数据新鲜度状态机均保留；`.5:50000` 电脑模拟器只用于隔离测试，真实 FPGA 联调时必须退出。
- F0 两位小数显示镜像 `e30d16c4…a4bac0e` 已完成构建、三段烧录校验和 100 帧烟测；它不替代绑定 `d819641d…d8207007` 的完整 M8/M9 20 例审计。
- CP2102N 串口设备号曾从 `/dev/ttyUSB0` 变化为 `/dev/ttyUSB1`。后续必须优先使用 `/dev/serial/by-id/` 稳定路径，不得把 `ttyUSB0` 写死为设备身份。
- 当前构建只认仓库根目录的 `main`。正式取证必须同时冻结源码差异、ELF/BIN 哈希和 FPGA 产物身份；`94dab8f-dirty` 只是一段历史基线，不能单独重建当时的未提交改动。
...
这代表“FPGA → ESP32-P4 数字链 PASS”。模拟前端、AD9226、滤波和 200 mVpp 干扰在最终标定/M12 的各自范围内已有独立证据；仍需按 G 题要求现场留证的是屏幕尺寸、单路 5 V 实物结构，以及按键到真实 panel flush 的 2 秒边界。
