...
这里是本 worktree 的恢复入口，不是普通 Git 发布目录。原始证据、截图和运行缓存仍被 `.gitignore` 排除；为保证可复现性，`test/` 下的夹具源码和 README 作为明确例外随合并提交。可合并、可引用的摘要在 [../docs/测试与证据索引.md](../docs/测试与证据索引.md)。
...
| `WaveBench使用体验与优化建议清单.md` | 基于本项目真实仪器联调、标定和留证过程整理的 WaveBench 改进建议。 | 跟踪；是开发归档版，不是对已归档证据的改写。 |
...
1. [../docs/测试与证据索引.md](../docs/测试与证据索引.md)：跨标定、审计、镜像和回放的合并入口。
...
5. [WaveBench使用体验与优化建议清单.md](WaveBench使用体验与优化建议清单.md)：给 WaveBench 后续演进和复用本实验台时参考的改进清单。
