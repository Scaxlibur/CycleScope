#!/usr/bin/env python3
"""Adversarial regression test for the current-v3 10k evidence parser.

The fixture source lives under ``tool-of-rei/test``.  It validates the fixed
formal logs, writes mutated log copies only to a temporary directory under
``/tmp``, and requires every falsified evidence pair to be rejected.
"""
...
import cslp_v3_long_stability_evidence as evidence
...
def expect_reject(
    ...
    with tempfile.TemporaryDirectory(
        prefix="cyclescope-long-evidence-adversarial-", dir="/tmp"
    ) as directory:
...
def main() -> int:
    ...
    tail_health = (
        ...
        "internal_free=118843 psram_free=28044452"
    )
    expect_reject(
    ...
    spectrum_bridge = evidence.common.SPECTRUM_UI_PATTERN.search(serial_text)
    if spectrum_bridge is None:
        raise RuntimeError("formal Spectrum UI bridge is missing")
    ...
    print(
        "CycleScope current-v3 10k adversarial evidence test passed: "
        "mutations=19 all_rejected"
    )
    return 0
