...
本仓当前用于 ESP32-P4 主线与资料交接；FPGA 仍在独立分支/工作树中维护。合并前的所有原始仪器、LAN、串���和���建档案均保留在本机，不会被这次整理删除、移动或改写。
...
- 当前工作树还包含 F0 显示到 0.01 Hz 的未提交改动。其新镜像完成构建、烧录哈希校验及真实 FPGA 100 帧启动/链路回归；完整 M8/M9 20 例审计绑定的是较早的应用 BIN。两者不能混作同一发布身份。
