...
- 缩写：CSLP
- 线上版本：1
- 文档状态：v0.1 的固定 8192 点基线已在 Zynq 与 ESP32-P4 落地并经实机证据验证；线上版本仍冻结为 1。动态帧长/运行时 Profile 切换不属于本版本已经交付的能力。
- 数据方向：波形由 Zynq 单向推送至 ESP32-P4，控制命令由 ESP32-P4 发起
- 消费策略：latest-frame-wins；尚未被业务层取得的旧完整帧允许被新完整帧覆盖

本文定义线上字节格式、分片规则、控制握手、接收缓冲和异常处理。当前比赛使用的 ADC、抽取倍数、帧长、滤波器及两端职责见 [CSLP G题采样与处理 Profile v0.1](./CSLP-G题采样与处理-Profile-v0.1.md)。构建身份、实机运行和原始测量证据见 [验收证据索引](../docs/验收证据索引.md)。
...
2. 同一帧内，满 MTU UDP 数据报的发送起点间隔建议不小于 150 us，防止 1G → 100M 交换机端口形成微突发。

当前 12 个分片按 150 us 起点间隔发送，整帧约 1.8 ms 内排完，远小于 50 ms 投递周期。直连和经交换机两种拓扑都必须抓包实测，再决定是否收紧分片间隔。不得依赖交换机缓存吸收整帧突发。
...
- Zynq PS GEM + lwIP 的实际发送抖动、缓存和 150 us 分片间隔。
- P4 lwIP pbuf、socket 接收缓存和 FFT 工作区的最终配置。
- 数字滤波器系数、定点位宽、群延迟、通带误差和阻带衰减。
- 模拟前端增益、ADC OTR 行为及 `scale_uV_per_lsb/offset_uV` 的校准结果。
- 直连及 1G → 100M 交换机拓扑下的长时间丢包率。
