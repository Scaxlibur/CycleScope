...
- ESP32-P4 已提交锚点：`main@e81be7a`，包含真实 CSLP/LAN 分析显示链及 1～8 条动态频谱；其后的 measurements 单位格式优化仍是待人工确认、待提交候选
- 最近更新：2026-08-01
...
`516/-6761`只是 100 kHz 标量，完整频率响应仍绑定在 `response.csv`，不能把
`CALIBRATED` 解释为固件已经做了逐频逆补偿。重新校准时允许产生新 ID，本表不是永久
Profile 常量。
...
- `main@e81be7a` 是当前已提交 P4 产品锚点；它包含正式 `.2` 对端、协议/接收门禁、
...
- `e81be7a` 之后的 measurements 候选把 Vpp/RMS 格式改为两位小数 mV、F0 和谱线频率
  改为整数 Hz；它已完成构建、烧录和真实 FPGA 200 帧回归，但在实际面板上的字号、
  间距与裁切仍待人工确认，尚不属于已提交锚点。
- 真实P4联合验收已由用户确认通过；详细帧数、哈希和证据路径未导入当前FPGA
  worktree，故这里不虚构数值。真实panel flush的2秒取证和`close()`返回失败后的fd
  所有权独立注入仍按各自证据边界记录。
