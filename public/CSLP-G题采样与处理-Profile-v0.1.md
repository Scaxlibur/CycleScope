...
- 状态：Profile v0.1 线上参数继续冻结；固定 8192 点两板主链、P4 逐频幅值补偿、真实链路与最终 P4 20例审计均已有证据。FPGA 唯一最终基线为 `fpga-v1.0.0@038e981`，整机物理显示时限仍需最终人工取证。
- FPGA 冻结锚点：签名 annotated tag `fpga-v1.0.0`，指向提交 `038e981`
- ESP32-P4 构建身份：完整20例审计绑定 BIN `d819641d…d8207007`；随后 F0 两位小数显示构建为 `e30d16c4…a4bac0e`，仅完成启动/100帧烟测，未取代前者的完整审计。
- 最近更新：2026-08-01
...
最终 `fit-v2` 使用36个训练 case 后再以7个独立 holdout 验证；holdout 最大幅值/频率
误差为 `0.320542 mV / 234.985 Hz`。P4 生成资产 `C5DCDE41` 只在校准清单、频响、
不确定度、holdout报告和 SHA-256 全部匹配时启用。协议中的 `516/-6761` 仍只是上游
标量元数据；`CALIBRATED` 单独不等于逐频补偿，P4还必须装载匹配的运行时资产。重新
校准时允许产生新 ID，本表不是永久 Profile 常量。
...
该轮具体帧数、哈希和证据路径尚未导入当前FPGA worktree，本文不补写未经本地核对的
数字。
...
- 当前 P4 源包含正式 `.2` 对端、协议/接收门禁、三缓冲、ANSI FFT、1P/3P、0～500 kHz
  频谱、动态谱线视窗、LIVE/STALE 状态机，以及 `C5DCDE41` 逐频幅值补偿。
- 主机与板上回归覆盖黄金包、控制幂等、非法头/CRC/session/config/元数据/分片、三缓冲
  所有权、socket恢复和带外拒绝；故障帧不会越过正式分析门禁。
- 真实 FPGA/P4 链已完成 `HELLO → CONFIG_SET → ENABLE_PUSH → measurement → UI LIVE`。
  M12 的60例工程矩阵、最终标定和最终 M8/M9 20例审计分别见
  [public/测试与证据索引](测试与证据索引.md) 与 [验收证据索引](../docs/验收证据索引.md)。
- 完整20例审计绑定 BIN `d819641d…d8207007`；F0 两位小数显示构建绑定
  `e30d16c4…a4bac0e`，仅完成构建/烧录/启动和100帧烟测，不应被写成同一发布身份。
- 真实 panel flush 的2秒端到端取证仍待人工完成；`close()` 返回失败后的 fd 所有权独立
  注入也仍是未关闭的工程风险。
...
- [x] 真实P4主端`.3:50001`已与正式FPGA`.2:50000`完成握手、首帧、100帧及连续
  600帧零错误分析/UI闭环。
- [x] 固定8192点的P4联合验收已由用户确认通过；详细帧数、哈希和证据路径尚未导入
  当前FPGA worktree，因此这里只记录结论，不补写未核对数字。
...
- [x] 固定8192点P4联合验收已由用户确认通过；详细证据尚未导入当前FPGA worktree。
...
- P4算法已在`main@e81be7a`冻结为8192点ANSI FFT、Hann窗、三点初值、直接正弦投影
...
- P4已冻结`SO_RCVBUF=64 KiB`、UDP/tcpip mailbox各64及三帧槽；剩余网络项是
  ...
  10,000帧双发长稳。
- `main@e81be7a`之后的measurements `mV/Hz`格式候选尚待实际面板视觉确认和提交；
  `v1.0.0@61eb0dc`不得静默移动，也不能被写成包含8谱线或该格式候选。
